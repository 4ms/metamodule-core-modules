...
struct Cache {
	...
	uint32_t map_buffer_to_cache(uintptr_t buffer_point, uint8_t sampleByteSize, CircularBuffer *b) const {
	...
	uintptr_t map_cache_to_buffer(uint32_t cache_point, uint8_t sampleByteSize, CircularBuffer *b) const {
		if (cache_point < low)
			return b->min; // error condition

		// Find how far ahead the cache_point is from the start of the cache
		uintptr_t p = cache_point - low;
