...
// Stereo "fade out" gain: pressing the Fade Out button ramps the gain from 1 to
// 0 over the Fade Time; the Shape knob morphs the ramp from linear (0) toward
// logarithmic (+1). The Reset button forces the gain back to 1 (full open).
// Audio Right is normalled to Audio Left when its jack is unpatched.
class FadeOutCore : public SmartCoreProcessor<FadeOutInfo> {
	using Info = FadeOutInfo;
	using ThisCore = FadeOutCore;
...
public:
	...
	void update() override {
		...
		// Fade Out button LED follows the gain; Reset LED flashes briefly on reset.
