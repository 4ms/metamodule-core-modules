...
class MWAVPCore : public SmartCoreProcessor<MWAVPInfo> {
	using enum MWAVPInfo::Elem;
...
public:
	...
	void update() override {
		...
		std::array<float, NumChannels> chan_out{};
		...
		// Each wav channel plays out its corresponding output; unused channels are silent
		auto num_chans = stream.num_channels();
		setOutput<Out1>(num_chans > 0 ? chan_out[0] * 5.f : 0.f);
		setOutput<Out2>(num_chans > 1 ? chan_out[1] * 5.f : 0.f);
		setOutput<Out3>(num_chans > 2 ? chan_out[2] * 5.f : 0.f);
		setOutput<Out4>(num_chans > 3 ? chan_out[3] * 5.f : 0.f);
		setOutput<Out5>(num_chans > 4 ? chan_out[4] * 5.f : 0.f);
		setOutput<Out6>(num_chans > 5 ? chan_out[5] * 5.f : 0.f);
		setOutput<Out7>(num_chans > 6 ? chan_out[6] * 5.f : 0.f);
		setOutput<Out8>(num_chans > 7 ? chan_out[7] * 5.f : 0.f);
	}
...
private:
	...
	StreamResampler resampler{NumChannels};
	...
	static inline bool was_registered = register_module<MWAVPCore, MWAVPInfo>("4msCompany");
};
