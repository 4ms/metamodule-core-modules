...
public:
	TapoCore()
		...
		delay.tap_modulo_observable_.set_observer([this] { onTapDetected(); });
...
// just ignore
		...
		printf("Loaded: Repeat %u, Current Slot %d, Sync %u, NumbSlots %u\n",
			   saveState.repeat,
			   saveState.current_slot,
			   saveState.sync,
			   saveState.slots.size());
#endif
	...
	void populateSaveState() {
		...
		saveState = SaveState_t{.repeat = delay.repeat() > 0.0f,
...
private:
	void packUnpackBlockBuffers() {
		...
		if (isPatched<AudioOut2Out>()) {
			setOutput<AudioOut1Out>(float(audioBufferTX[audioBufferFillCount].l) / 32768.0 *
									AudioOutputFullScaleInVolt);
			setOutput<AudioOut2Out>(float(audioBufferTX[audioBufferFillCount].r) / 32768.0 *
									AudioOutputFullScaleInVolt);
		} else {
			setOutput<AudioOut1Out>(
				(float(audioBufferTX[audioBufferFillCount].l) + float(audioBufferTX[audioBufferFillCount].r)) / 2 /
				32768.0 * AudioOutputFullScaleInVolt);
		}
	}
	...
	void sideloadDrivers() {
		...
		};
		{
			...
			adc.set(ADC_SCALE_CV, BipolarCVInputFunc(val));
		}
		{
			auto val = getInput<FeedbackJackIn>().value_or(0);
			adc.set(ADC_FEEDBACK_CV, BipolarCVInputFunc(val));
		}
		{
			auto val = getInput<ModulationJackIn>().value_or(0);
			adc.set(ADC_MODULATION_CV, BipolarCVInputFunc(val));
		}
		{
			auto val = getInput<DryWetJackIn>().value_or(0);
			adc.set(ADC_DRYWET_CV, BipolarCVInputFunc(val));
		}
		...
		{
			auto val = getInput<VelocityIn>().value_or(0.f);
			adc.set(ADC_VEL_CV, VelocityCVInputFunc(val));
		}
		...
		//
		{
			auto val = getInput<RepeatJackIn>().value_or(0.f);
			gate.set(GATE_INPUT_REPEAT, val > GateInputThresholdInVolt);
		}
		{
			auto val = getInput<TapIn>().value_or(0.f);
			adc.set(ADC_TAPTRIG_CV, val > GateInputThresholdInVolt);
		}
		{
			auto val = getInput<ExtClockIn>().value_or(0.f);
			adc.set(ADC_CLOCK_CV, val > GateInputThresholdInVolt);
		}
		...
		//
		adc.set(ADC_FSR_CV,
				getState<TapSensorButton>() == MomentaryButton::State_t::PRESSED ? TapSensorPressedIntensity : 0.0);
