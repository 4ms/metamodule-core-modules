...
public:
	...
	void sideloadDrivers() {
		...
		auto Convert3WaySwitchStateFunc = [](auto inState) -> LDKit::Mocks::SwitchPos {
			switch (inState) {
				case Toggle3pos::State_t::DOWN:
					return LDKit::Mocks::SwitchPos::Down;
				case Toggle3pos::State_t::UP:
					return LDKit::Mocks::SwitchPos::Up;
				case Toggle3pos::State_t::CENTER:
					return LDKit::Mocks::SwitchPos::Center;
				default:
					return LDKit::Mocks::SwitchPos::Invalid;
			}
		};
	...
	void handleAltParameters() {
		...
		params.settings.rev_jack =
			getState<Mapping::ReverseInputModeAlt>() == 0 ? LDKit::GateType::Trig : LDKit::GateType::Gate;
		params.settings.inf_jack =
			getState<Mapping::HoldInputModeAlt>() == 0 ? LDKit::GateType::Trig : LDKit::GateType::Gate;
		...
		params.settings.crossfade_samples =
			AdaptLengthToSampleRateFunc(CrossfadeSamples[getState<Mapping::CrossFadeTimeAlt>()]);
		params.settings.crossfade_rate = params.settings.calc_fade_increment(params.settings.crossfade_samples);
		params.settings.write_crossfade_samples =
			AdaptLengthToSampleRateFunc(WriteCrossfadeSamples[getState<Mapping::CrossFadeTimeAlt>()]);
		params.settings.write_crossfade_rate =
			params.settings.calc_fade_increment(params.settings.write_crossfade_samples);

		static constexpr std::array<PingMethod, 5> PingMethods = {PingMethod::LINEAR_AVERAGE_4,
...
private:
	void packUnpackBlockBuffers() {
		...
		inBlock[audioBufferFillCount] = {InputConversionFunc(getInput<Mapping::ReturnInput>().value_or(0)),
										 InputConversionFunc(io.audioIn)};
